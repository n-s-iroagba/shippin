...
export default function ShippingStageCrudPage() {
  const { data: shippingStages, loading, error } = useGetList<ShippingStage>('shipping-stages');
  const [stageForDocument, setStageForDocument] = useState<ShippingStage | null>(null);
  const [stageForPayment, setStageForPayment] = useState<ShippingStage | null>(null);
  const [documentType, setDocumentType] = useState<'supportingDocument' | 'paymentReceipt'>('supportingDocument');
  ...
  if (error) {
    return (
      <AdminOffCanvas>
        <ErrorComponent message={error || "Failed to load shipping stages"} />
      </AdminOffCanvas>
    );
  }

  const handleViewDocument = (stage: ShippingStage, type: 'supportingDocument' | 'paymentReceipt') => {
        ...
        <div className="container mx-auto p-4 bg-blue-50 min-h-screen">
          <div className="max-w-6xl mx-auto">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6 sm:mb-8">
              <h1 className="text-2xl sm:text-3xl font-bold text-blue-900">Shipping Stages</h1>
            ...
            {(!shippingStages || shippingStages.length === 0) ? (
              <div className="bg-blue-50 p-8 rounded-2xl border-2 border-blue-100 text-center max-w-md mx-auto">
                ...
                <h3 className="text-lg font-semibold text-blue-900 mb-2">No Shipping Stages Yet</h3>
            ...
            {stageForDocument && (
              <DocumentModal
                ...
                title={`${documentType === 'supportingDocument' ? 'Supporting Document' : 'Payment Receipt'} - ${stageForDocument.title}`}
