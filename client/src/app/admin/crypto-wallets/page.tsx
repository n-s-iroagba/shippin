...
export default function CryptoWalletCrudPage() {
  const { data: wallets, loading, error } = useGetList<CryptoWallet>('crypto-wallets');
  ...
  if (error) {
    return (
        ...
        <ErrorAlert message={error || "Failed to load crypto wallets"} />
        ...
        <div className="container mx-auto p-4 bg-blue-50 min-h-screen">
          <div className="max-w-6xl mx-auto">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6 sm:mb-8">
              <h1 className="text-2xl sm:text-3xl font-bold text-blue-900">Crypto Wallets</h1>
              <button
                name='addNewWallet'
            ...
            {(!wallets || wallets.length === 0) ? (
              <div className="bg-blue-50 p-8 rounded-2xl border-2 border-blue-100 text-center max-w-md mx-auto">
                ...
                <h3 className="text-lg font-semibold text-blue-900 mb-2">No Wallets Yet</h3>
            ...
            {walletToDelete && (
              <DeleteConfirmationModal
                ...
                type={'wallet'}
                message={`${walletToDelete.currency} wallet (${walletToDelete.label || 'No label'})`}
              />
            )}
          </div>
        </div>
