    ...
    return (
        <div className="bg-white text-black">
            ...
            <div className="space-y-2 text-black">
                ...
                <p className="rounded border-b-4 border-goldenrod p-2">
                    <strong>Sending Port:</strong> {shipmentDetails.shippingTakeoffAddress}
                </p>
                <p className="rounded border-b-4 border-goldenrod p-2">
                    ...
                    {shipmentDetails?.shipmentStatuses?.map((step: ShipmentStatus) => (
                        <li key={step.id} className="flex justify-evenly border-b p-1">
                            <div className="flex flex-col w-[60%] justify-between">
                                ...
                                <span className="min-h-[40px] break-words whitespace-normal text-black text-center">
                                    {step.dateAndTime}
                                </span>
                                <span className="min-h-[40px] break-words whitespace-normal text-black text-center">
                                    {step.carrierNote}
            ...
            {showEditShipmentStatusModal && selectedShipmentStatus && (
                <EditShipmentStatusModal step={selectedShipmentStatus} onClose={() => setShowEditShipmentStatusModal(false)} />
            )}
            {showDeleteShipmentStatusModal && selectedShipmentStatus && (
                <DeleteShipmentStatusModal step={selectedShipmentStatus} onClose={() => setShowDeleteShipmentStatusModal(false)} />
            )}
        </div>
    );
};
